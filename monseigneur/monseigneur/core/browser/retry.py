...
def login_method(func):
    ...
    `core.browser.pages.LoginPage` will not raise `LoggedOut`, since it is
...
def retry_on_logout(exc_check=LoggedOut, tries=4):
    ...
    raises an exception of the type `core.browser.exceptions.LoggedOut`.
...
class RetryLoginBrowser(LoginBrowser):
    ...
    let logouts raise a `core.browser.exceptions.LoggedOut` exception that
    is handled with a retry, thanks to the `@retry_on_logout` decorator.

    The `core.browser.pages.LoginPage` will raise `LoggedOut` if the browser
